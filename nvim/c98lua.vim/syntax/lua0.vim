...
syn cluster luaExpr←   contains=luaCommentA←,luaConstant←,luaNumber←,luaString←,luaBuiltin←,luaTable←,luaOperator←,luaEllipsis←,luaComma←,luaFunc←,luaError←,luaWord←,luaUnop←,luaParen←
...
syn region  luaTable←    ↑         matchgroup=luaD← start=/{/                  end=/}/              contains=@luaExpr→,luaTableKey→
syn match   luaTableEq←  contained                  /=/                                                                        nextgroup=@luaExpr→
syn match   luaTableKey→ contained                  /\K\k*/                                                                    nextgroup=luaTableEq←
syn region  luaTableKey→ contained matchgroup=luaD← start=/\[/                 end=/]/              contains=@luaExpr→         nextgroup=luaTableEq←

syn match   luaWord←     ↑                          /\K\k*/                                                                    nextgroup=@luaSuffix←
syn region  luaParen←    ↑         matchgroup=luaD← start=/(/                  end=/)/              contains=@luaExpr→         nextgroup=@luaSuffix←
syn match   luaDot←      contained                  /[.:]/                                                                     nextgroup=luaWord←
syn region  luaIndex←    contained matchgroup=luaD← start=/\[/                 end=/\]/             contains=@luaExpr→         nextgroup=@luaSuffix←
syn region  luaCall←     contained matchgroup=luaD← start=/(/                  end=/)/              contains=@luaExpr→         nextgroup=@luaSuffix←
syn region  luaCall←     contained matchgroup=luaD← start=/{/                  end=/}/              contains=@luaExpr→         nextgroup=@luaSuffix←
syn match   luaUnop←     ↑                          /[#~-]\|\<not\>/                                                           nextgroup=@luaExpr←
syn match   luaBinop←    contained                  /[<>=~^&|*/%+-]/                                                           nextgroup=@luaExpr←
syn match   luaBinop←    contained                  ://\|>>\|<<\|\.\.\|[<>=~]=:                                                nextgroup=@luaExpr←
syn match   luaBinop←    contained                  /\<and\>\|\<or\>/                                                          nextgroup=@luaExpr←
...
syn region  luaString←   ↑                          start=/'/                  end=/'/              contains=luaEscape         nextgroup=@luaSuffix←
syn region  luaString←   ↑                          start=/"/                  end=/"/              contains=luaEscape         nextgroup=@luaSuffix←
...
syn match   luaCommentA← ↑                          /--.*$/                                         contains=luaTodo           nextgroup=@luaExpr←
syn region  luaCommentA← ↑                          start=/--\[\z(=*\)\[/      end=/\]\z1\]/        contains=luaTodo           nextgroup=@luaExpr←
syn match   luaCommentB← ↑                          /--.*$/                                         contains=luaTodo           nextgroup=@luaStmt←
syn region  luaCommentB← ↑                          start=/--\[\z(=*\)\[/      end=/\]\z1\]/        contains=luaTodo           nextgroup=@luaStmt←
syn match   luaCommentC← ↑                          /--.*$/                                         contains=luaTodo           nextgroup=@luaSuffix←
syn region  luaCommentC← ↑                          start=/--\[\z(=*\)\[/      end=/\]\z1\]/        contains=luaTodo           nextgroup=@luaSuffix←
...
syn region  luaIf←       ↑         matchgroup=luaD← start=/\<if\>/             end=/\ze\<then\>/    contains=@luaExpr→         nextgroup=luaIfBody←
syn region  luaIfBody←   contained matchgroup=luaD← start=/\<then\>/           end=/\<end\>/        contains=@luaStmt→,luaElse←
syn region  luaElse←     contained matchgroup=luaD← start=/\<elseif\>/         end=/\<then\>/       contains=@luaExpr→
syn region  luaElse←     contained matchgroup=luaD← start=/\<else\>/           end=/\>/

syn region  luaLoop←     ↑         matchgroup=luaD← start=/\<while\>/          end=/\ze\<do\>/      contains=@luaExpr→         nextgroup=luaDo←
syn region  luaLoop←     ↑         matchgroup=luaD← start=/\<for\>/            end=/\ze\<do\>/      contains=@luaExpr→,luaIn←  nextgroup=luaDo←
syn region  luaDo←       ↑         matchgroup=luaD← start=/\<do\>/             end=/\<end\>/        contains=@luaStmt→
syn match   luaIn→       contained                  /\<in\>\|,\|=/

syn region  luaLoop←     ↑         matchgroup=luaD← start=/\<repeat\>/         end=/\<until\>/      contains=@luaStmt→         nextgroup=@luaExpr→
